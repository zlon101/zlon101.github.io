[Liubai](https://github.com/yenche123/liubai) ：开源的微信机器人，在微信里面同时调用三个 AI 模型，完成各种任务。
...
逻辑层的异常捕获使用 App构造器里提供了onError 的回调
...
wx.login({
  success(res) {
    ...
    const APPID = '';
    const SECRET = '';
    let urlQ = `?appid=${APPID}&secret=${SECRET}&js_code=${res.code}&grant_type=authorization_code`;
    wx.request({
      url: 'https://api.weixin.qq.com/sns/jscode2session' + urlQ,
      success(res) {
