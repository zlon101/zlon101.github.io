...
mac 中在 `.zshrc` 中配置一下环境变量：

```shell
alias chrome-debug='/Applications/Google\ Chrome\ Dev.app/Contents/MacOS/Google\ Chrome\ Dev --user-data-dir="/xxx/chrome-dev-user" --disable-web-security'
```
...
**注意：**部分参数只有在指定了 `--user-data-dir` 的情况下才生效，所以建议都指定 `--user-data-dir `



用命令行参数指定代理服务器可以解决无法将【本地 IP+端口】代理到其他本地IP的其他端口的问题

[chrome 启动命令](https://himle.github.io/201609Chromium-%E5%91%BD%E4%BB%A4%E8%A1%8C%E5%90%AF%E5%8A%A8%E5%8F%82%E6%95%B0/) 

[官网](https://www.chromium.org/developers/how-tos/run-chromium-with-flags/) 

[翻译](https://www.cnblogs.com/gurenyumao/p/14721035.html) 
...
**二、 [SwitchyOmega](https://chrome.google.com/webstore/detail/proxy-switchyomega/padekgcemlokbadohgkifijomclgjgif?utm_source=chrome-ntp-icon) **
...
- 正向代理，页面请求转发给【代理服务器】，可以是 AnyProxy 或者 Nginx 等。
...
- mock 目录下编写 json 文件作为mock数据返回给前端
...
app.use(async (ctx) => {
  try {
    ...
    const filePath = path.join(mockDir, `${urlPath}.json`);
    ...
    console.error(err);
    ctx.body = { retCode: 0, msg: '没有mock数据' };
  }
});
...
# [whistle-npm包](https://segmentfault.com/a/1190000042416569) 
...
[Charles 注册](https://cloud.tencent.com/developer/article/1883862) 

[为什么你的 Charles 会抓包失败 - Android 版本](https://supercodepower.com/use-charles#4android-%E7%89%88%E6%9C%AC%E8%B6%8A%E9%AB%98https-%E6%8A%A5%E6%96%87%E8%B6%8A%E9%9A%BE%E6%8A%93) 
...
<span style="color:red">注意：</span> Chrome 和 Firefox 浏览器默认并不使用系统的代理服务器设置，而 Charles 是通过将自己设置成代理服务器来完成封包截取的，所以在默认情况下无法截取 Chrome 和 Firefox 浏览器的网络通讯内容。如果你需要截取的话，在 Chrome 中设置成使用系统的代理服务器设置即可，或者直接将代理服务器设置成 127.0.0.1:8888 也可达到相同效果。
...
在 chrome 里，如果浏览器不能验证网站的证书，会弹出警告界面，这时候输入 thisisnotsafe 就可以（`google.com/xx?q="thisisnotsafe"`）
...
[远程调试WebView、性能监视器、FPS实时监控性能](https://mp.weixin.qq.com/s?__biz=Mzg5ODA5NTM1Mw==&mid=2247484505&idx=2&sn=b4b116266be5fff08676b1d820fc4555&chksm=c06685cff7110cd9cc62310130e0a03b6a74cbbfdc270dbcf1163f145377d289fae9e3de5976&mpshare=1&scene=1&srcid=&sharer_sharetime=1570841366248&sharer_shareid=c0fa4bb765d12545f4439ab827814978&key=975560f4a5cae610c574db32525465566a656df6645e89b976c27fd903d4a43964010a64f72d7bd438277fdf6806ff4fce1176242c45bb7b4fc85793918f9591ca55c75525b53e1487df7e6067dcecbe&ascene=1&uin=Mjc2NDI1NDU2NA%3D%3D&devicetype=Windows+7&version=62060833&lang=zh_CN&pass_ticket=nIukjmaPQXgp2q0hcW%2B%2Bahc5NkfqJkbvZat5TcXTy6ffxhB2rGnoEuAj8V8LSL2H) 

# source map

利用chrome的 add  sourcemap 功能调试线上bug

# Android 设备

[管理 Android 设备的方式 - Android Debug Bridge](https://app.tangoapp.dev/) 
