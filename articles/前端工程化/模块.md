...
- 编译时加载，静态分析，import 命令提前，多次加载只会执行一次。生成**只读引用**，运行时再根据引用到模块中取值。接口与模块内部的值是动态绑定。
...
1. 依赖处理：AMD在定义模块时就需要声明所有的依赖关系，而CMD则是在运行时才会处理依赖关系。
2. 执行顺序：AMD会等待所有依赖模块都加载完成后再执行模块代码，而CMD则是遇到依赖模块就立即执行其代码。
